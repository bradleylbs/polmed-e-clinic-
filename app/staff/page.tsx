...
export default function StaffHomePage() {
  ...
  const [initialView, setInitialView] = useState<ViewMode | null>(null)
  const [routeToEdit, setRouteToEdit] = useState<ApiRoute | null>(null)
  ...
  // Read deep-link view from query string, e.g. /staff?view=appointments
  useEffect(() => {
    try {
      if (typeof window === 'undefined') return
      const usp = new URLSearchParams(window.location.search)
      const viewParam = (usp.get('view') || '').toLowerCase()
      let mapped: ViewMode | null = null
      switch (viewParam) {
        case 'appointments':
        case 'routes':
          mapped = 'routes'
      ...
      setInitialView(mapped)
    } catch (e) {
      console.warn('Failed to parse view from query params', e)
      setInitialView(null)
    }
  }, [])

  // Apply initialView after login or if already logged in
  useEffect(() => {
    if (!isInitializing && user && initialView) {
      setViewMode(initialView)
    }
  }, [isInitializing, user, initialView])
  ...
  setUser(newUser)
  setViewMode(initialView || "dashboard") // Respect deep-linked view if present
  ...
  const renderContent = () => {
    try {
      switch (viewMode) {
        ...
        case "routes":
          return <RouteList userRole={user.role} onRouteSelect={handleRouteSelect} onNewRoute={handleNewRoute} onEditRoute={handleEditRoute} />
