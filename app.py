...
PALMED Mobile Clinic ERP - Flask Application Entry Pointfrom flask_cors import CORS

This file is at the root to work with Gunicorn's default lookup.from werkzeug.security import generate_password_hash, check_password_hash

It imports the Flask app from scripts/app.pyimport mysql.connector

"""from mysql.connector import Error

import jwt

import sysfrom datetime import date, datetime, timedelta, timezone

import osfrom functools import wraps

import os

# Add scripts folder to Python path so we can import the appimport logging

sys.path.insert(0, os.path.join(os.path.dirname(__file__), 'scripts'))from typing import Dict, List, Set

import uuid

# Import the Flask appimport json

from app import appfrom dotenv import load_dotenv



# Make app available at module level for Gunicorn# Load environment variables from .env file
...
if __name__ == '__main__':# Configure logging
    ...
    # Development serverlogging.basicConfig(level=logging.INFO)

    app.run(debug=True)logger = logging.getLogger(__name__)
